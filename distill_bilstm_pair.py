...
from bilstm_pair import BiLstmPair
...
def main_train():
    ...
    parser.add_argument('--task', required=True, choices=['xnli', 'snli'], help='task name')
    ...
    parser.add_argument('--max_seq_len', type=int, default=64, help='max sequence length')
    ...
    model = BiLstmPair(args.embed_dim, args.hidden_dim, args.fc_dim,
...
def main_tune():
    ...
    for lr in [5e-03, 1e-03, 5e-04, 3e-04, 2e-03]:
        for _ in range(5):
            ...
            dev_score = run(args, model, train_dataset, val_dataset, device, silent=True)
